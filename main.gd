...
func spawn_block():
	...
	#var block_num = 1 # t block helps show proper coord layout
	...
	for block_row in block_matrix:
		for block_col in block_row:
			print("row ", posVertical, " spot ", posHorizontal, " put a ", block_col)
			game_grid[posVertical][posHorizontal] = block_col
			if block_col != 0:
...
func freeze_moving_block():
	...
	for child in $MovingBlockController.get_children():
		child.queue_free()
	draw_game_grid()
	print("updated game grid:", game_grid)
	spawn_block()
	pass
...
func draw_block(horizontal, vertical, block_type):
	...
	block.get_children()[0].get_mesh().surface_get_material(0).albedo_color = BlockCoords.block_colors[moving_block_type]
	return block
